...
export default async function handler(req, res) {
  ...
  const headers = {
	...
	await sendTransaction(api,contract,signerAddress, "UpdateFhir",[Number(userid),walletaddress, patient_details["name"][0]['family'], givenname, identifier, patient_details["telecom"][0]["value"].toString(), patient_details["gender"], patient_details["birthDate"], decodedDiseaseId, patientid]);


	res.status(200).json({status: 200, value: "Updated!",bodies: [Number(userid),walletaddress, patient_details["name"][0]['family'], givenname, identifier, patient_details["telecom"][0]["value"].toString(), patient_details["gender"], patient_details["birthDate"], decodedDiseaseId, patientid]});
}
